...
com 2 graus de liberdade, parâmetro de não-centralidade $\lambda =
...
m <- mean(dados$SQRTVU)
s <- sd(dados$SQRTVU)
histogram(sqrt(s^2*rchisq(300, 2, ncp = (m/s)^2)))
...
plotDist("chisq", df = 2, ncp = (m/s)^2, add = TRUE)
```


```{r, fig.cap = "Valores Unitários de Lotes com distribuição lognormal.", echo = FALSE, eval = FALSE}
library(fitdistrplus)
fln <- fitdist(dados$VU/s^2, "chisq", start = list(df = 2, ncp = (m/s)^2))
par(mfrow = c(1, 2))
denscomp(fln)
qqcomp(fln)
```
